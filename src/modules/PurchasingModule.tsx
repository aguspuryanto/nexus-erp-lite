...
import { Transaction, TransactionItem, Partner, Product } from '../types';
...
export const PurchasingModule = () => {
  ...
  const [type, setType] = useState<'PR' | 'PO' | 'INVOICE_IN'>('PR');
  ...
  const fetchTransactions = () => {
    axios.get('/api/transactions')
      .then(res => setTransactions(res.data.filter((t: any) => ['PR', 'PO', 'INVOICE_IN'].includes(t.type))));
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button 
            onClick={() => { setType('INVOICE_IN'); setShowForm(true); }}
                ...
                <tr key={t.id} className="hover:bg-zinc-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm font-bold">
                    <span className={cn(
                      ...
                      t.type === 'PR' ? "bg-blue-50 text-blue-600" : 
                      t.type === 'PO' ? "bg-amber-50 text-amber-600" : "bg-emerald-50 text-emerald-600"
                    )}>
                      {t.type}
                    </span>
                  </td>
